...
const sagittarius = () => {
  ...
  setTimeout(function () {
    ...
    var leftDistance =
      getLeftDistance(triangle) + getWidth(triangle) / 2 - strokeWidth / 2;
...
const scorpio = () => {
  ...
  setTimeout(function () {
    ...
    fourthCircle = splitCellHorizontal(
      circle,
      getWidth(circle) * 2 + strokeWidth * -3,
      animationInterval
    );
  }, animationInterval);
  ...
  setTimeout(function () {
    ...
    moveElement(
      triangle,
      getLeftDistance(fourthCircle) +
        getWidth(fourthCircle) -
        getWidth(triangle) * (3 / 4),
      getTopDistance(fourthCircle) +
        getHeight(fourthCircle) / 2 -
        getHeight(triangle) +
        1,
      animationInterval
    );
...
const virgo = () => {
  ...
  setTimeout(function () {
    ...
    mask.style.top =
      getTopDistance(fourthCircle) + getHeight(fourthCircle) / 2 + 1 + "px";
...
const leo = () => {
  ...
  setTimeout(function () {
    ...
    moveElement(
      circle,
      getLeftDistance(circle) + strokeWidth,
      getTopDistance(circle) + getHeight(circle) - 2 * strokeWidth,
      animationInterval
    );
  ...
  setTimeout(function () {
    ...
    moveElement(
      circles[2],
      getLeftDistance(circles[2]),
      getTopDistance(circles[2]) + 2 * getHeight(circle) - 2 * strokeWidth,
      animationInterval
    );
...
const capricorn = () => {
  ...
  setTimeout(function () {
    ...
    moveElement(
      circle,
      getRightDistance(triangle) - getWidth(circle) - strokeWidth * 0.84,
      getBottomDistance(triangle) - getHeight(circle),
      animationInterval
    );
  }, animationInterval * 2);
  ...
  setTimeout(function () {
    ...
    moveElement(
      secondCircle,
      getLeftDistance(triangle),
      getBottomDistance(triangle) - getHeight(secondCircle) / 2,
      animationInterval
    );
  }, animationInterval * 4);
  ...
  setTimeout(function () {
    ...
    mask.style.top =
      getTopDistance(triangle) + getHeight(triangle) * (2 / 3) + 5 + "px";
...
const libra = () => {
  ...
  setTimeout(function () {
    ...
    positionElement(
      bar,
      getBottomDistance(circle) - strokeWidth,
      getHorizontalCentre(circle) - originalCircleWidth / 2
    );
    ...
    positionElement(
      bar2,
      getBottomDistance(circle) + getHeight(circle) - strokeWidth * 2.5,
      getHorizontalCentre(circle) - originalCircleWidth / 2
    );
...
const gemini = () => {
  ...
  moveCellVertical(
    circle,
    getHeight(circle) / 2 + gapBetweenCircles / 2,
    animationInterval
  );
  ...
  setTimeout(function () {
    ...
    positionElement(
      bar1,
      getBottomDistance(topCircle) - strokeWidth,
      getLeftDistance(circle) + getWidth(circle) * 0.3
    );
    ...
    positionElement(
      bar2,
      getBottomDistance(topCircle) - strokeWidth,
      getLeftDistance(circle) + getWidth(circle) * 0.6
    );
    circle.parentElement.appendChild(bar2);
  }, animationInterval * 3);
...
const pisces = () => {
  ...
  setTimeout(function () {
    ...
    var topDistance =
      getTopDistance(circle) + getWidth(circle) / 2 - strokeWidth / 2;
    positionElement(crossBar, topDistance, leftDistance);
...
function maskPartOfCircle(element, side, percentToMask) {
  ...
  switch (side) {
    ...
    case "bottom":
      ...
      mask.style.top =
        elemTopValue + elemHeight * ((100 - percentToMask) / 100) + "px";
      mask.style.left = elemLeftValue + "px";
    ...
    case "right":
      ...
      mask.style.top = elemTopValue + "px";
      mask.style.left =
        elemLeftValue + elemWidth * (percentToMask / 100) + "px";
      mask.style.height = elemHeight + 5 + "px";
      break;

    default:
      console.log(
        "Please enter top, bottom, left or right. You entered: " + side
      );
  }
...
const splitCellCancer = (elem, gap, animationInterval) => {
  ...
  root.style.setProperty(
    "--left-final",
    getLeftDistance(elem) + getWidth(elem) + gap + "px"
  );
...
function fanOutHorizontal(elem, number, strokeWidth, animationInterval) {
  ...
  for (var i = 1; i < number; i++) {
    let newElem = splitCellHorizontal(
      elem,
      getWidth(elem) * (i - 1) + strokeWidth * (i * -1),
      animationInterval
    );
...
function lineFromTriangle(elem, strokeWidth) {
  ...
  let root = document.documentElement;
  root.style.setProperty(
    "--triangle-line-clip-path",
    "polygon(50% 0, " +
      halfWidthPlus50 +
      "% " +
      triangleHeight +
      "%, " +
      baseStrokeWidthAsPercent +
      "% 100%, 0 100%)"
  );
...
function hollowFlippedTriangle(elem, strokeWidth) {
  ...
  bottomDot.y =
    100 -
    relativeStrokeWidth / Math.cos(degreesToRadians(36.87)) / Math.sqrt(3 / 2);
  bottomDot.x = 50 - relativeStrokeWidth / 2;
  topRightDot.y = relativeStrokeWidth;
  topRightDot.x =
    100 -
    relativeStrokeWidth / 2 -
    relativeStrokeWidth / Math.cos(degreesToRadians(26.565));
  ...
  let root = document.documentElement;
  root.style.setProperty(
    "--flipped-hollow-triangle-clip-path",
    hollowPolygonString
  );
  root.style.setProperty(
    "--flipped-prehollow-triangle-clip-path",
    preHollowPolygonString
  );
...
function sizeOfCapricornCircleGivenTriangleWidth(triangleWidth) {
  return (
    (triangleWidth * Math.sin(degreesToRadians(63.435))) /
    (Math.sin(degreesToRadians(63.435)) +
      2 *
        Math.sin(degreesToRadians(53.13)) *
        Math.cos(degreesToRadians(26.565)))
  );
}
...
function finishAnimation(astroSign) {
  ...
  setTimeout(function () {
    ...
    astroText.innerHTML =
      "<h4>" +
      '<span id="starsign-name">' +
      starSignQualities[astroSign].name +
      "</span>" +
      " (" +
      starSignQualities[astroSign].dates +
      ")</h4>" +
      "<p>" +
      starSignQualities[astroSign].description +
      "</p>";
